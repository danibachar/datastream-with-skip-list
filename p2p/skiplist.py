...
from itertools import chain, dropwhile, dropwhile, count, repeat
from threading import Lock
import random, collections

# Based on - https://github.com/ZhukovAlexander/py-skiplist
def geometric(p):
...
class _Skipnode(object):
    ...
    def disconnect(self, client):
      try:
        ...
        if DEBUG:
          print("Trying to disconnect {} from {}, but it is not in the connections list {}".format(client._id, self.key, self.data))
...
class Skiplist(SkiplistAbstractBase, collections.MutableMapping):
    ...
    def _insert(self, key, data):
        node = super(Skiplist, self)._insert(key, data)
        self._size += 1
        return node
    ...
    def __str__(self):
      ...
      for level in range(self._height()):
        res += "Node at level {} are:\n".format(level)
        res += ', '.join('{key}: {value}'.format(key=node.key, value=node.data) for node in self._level(level=level))
        res += "\n"
      return res
